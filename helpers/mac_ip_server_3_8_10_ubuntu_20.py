...
from typing import List, Dict, Any           # <-- added for 3.8 compatibility
...
def classify_ip(ip: str) -> str:
    ...
    if a in ipaddress.IPv4Network("224.0.0.0/24"):
        return "multicast_local"
    if a.is_multicast:
        return "multicast_admin" if a in ipaddress.IPv4Network("239.0.0.0/8") else "multicast_global"
    if ip.endswith(".255"):
        return "broadcast_subnet"
...
def ping(host: str):
    cmd = ["ping", "-c", "1", "-W", "1", host]  # Linux-style timeout
...
def parse_arp(now: int) -> List[Dict[str, Any]]:
    """Return list of dicts with ip, mac, kind, seen (Linux-compatible)."""
    out = subprocess.run(["arp", "-an"], capture_output=True, text=True).stdout

    current_macs = set()
    table: List[Dict[str, Any]] = []

    # Regex for: ? (192.168.1.17) at d0:03:df:30:fb:4b
    pattern = re.compile(r"\(\s*([\d.]+)\s*\)\s+at\s+([0-9a-f:]{17})", re.I)
...
if __name__ == "__main__":
    ...
    print(
        f"Serving http://{HOST}:{PORT}/?demo=1  subnet={subnet_cidr}  "
        f"refresh={DEFAULT_REFRESH}s"
    )
